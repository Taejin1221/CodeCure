 ...
 * Compile with gcc
 * 
 * @title CodeCure_Problem_Pointer_Solution
 * @author Taejin
...
/* =============== ProtoType ===============*/
void Swap(int* ptr1, int* ptr2); // 매개변수 직접 작성
int StrLen(char* str); // 매개변수 직접 작성
bool Collision();
...
bool Collision() {
	...
	char* str = temp;
	char input[10];
	printf("Input str: "); scanf("%s", input); // CodeCure
...
bool CE() {
	...
	printf("Input (charAnswer): "); scanf("%x", &charAnswer); // 12
	charPtr = num;
	charPtr += 2;
	...
	printf("Input (intAnswer): "); scanf("%x", &intAnswer); // 65432109
	intPtr = charPtr;
	intPtr += 2;
	if ((*intPtr) != intAnswer) {
	...
	printf("Input (shortAnswer): "); scanf("%x", &shortAnswer); // 0987
	shortPtr = intPtr;
	shortPtr += 2;
	...
	printf("Input (long long int Answer): "); scanf("%llx", &lliAnswer); // 0987654321098765
	lliPtr = num + 1;
	if ((*lliPtr) != lliAnswer) {
